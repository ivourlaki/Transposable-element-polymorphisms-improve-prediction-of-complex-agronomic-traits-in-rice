...
###                          PREDICTION USING RLX/RIX MARKERS                              ###
...
#--- load the marker matrices. We use only the RLX/RIX here -----------------------------
...
somePDFPath = "BAYESCADMARO_mite.pdf"
...
for (i in 1:11) {
  ...
  fm1 = BGLR(y=yNA,ETA=list(ETA01=list(X=V1,model="FIXED"),ETA1=list(X=rlx, model='BayesC', probIn=pi, counts=p0)), nIter=nIter,saveAt=sprintf("./fmgBAYESCaroadmrlx_y%.0f_",i),verbose=F)
  ...
  par(new=TRUE)
  plot(yHat[tstARO]~y1[tstARO], col="BLUE", xlim=tmp,ylim=c(-2,2),xlab="Observed",ylab="Predicted",main=c(names[i],sprintf("ARO,ADM rlx BayesC, cor%.02f", corel1)))
  par(new=TRUE)
...
png(file=sprintf("./corr_BAYESC_rlx_AROADM.png"))
...
save.image("BAYESCrlxAROADM.RData")
rm(list = ls())
...
for (i in 1:11) {
  ...
  fm2 = BGLR(y=yNA,ETA=list(ETA01=list(X=V1,model="FIXED"),ETA1=list(X=rlx, model='BayesC', probIn=pi, counts=p0)), nIter=nIter,saveAt=sprintf("./fmgBAYESCINDrlx_y%.0f_",i),verbose=F)
  ...
  y1=as.numeric(y)
  tmp<-range(c(yHat,y),na.rm = T) 
  plot(yHat[tst]~ y1[tst], col="red", xlim=tmp,ylim=c(-2,2),xlab="Observed",ylab="Predicted",main=c(names[i],sprintf("IND rlx BayesC, cor%.02f", corel2)))
  par(new=TRUE)
...
row.names(all.phenotypes.COR)<-names
colnames(all.phenotypes.COR)<- c("correlation")
png(file=sprintf("./corr_BAYESc_rlx_IND.png"))
grid.table(all.phenotypes.COR)

dev.off()


save.image("BayesCrlxIND.RData")
rm(list = ls())
