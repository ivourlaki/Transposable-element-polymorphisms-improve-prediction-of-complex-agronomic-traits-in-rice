...
#        Covariance matrix has been generated by AGHmatrix package using three marker as input data
...
###  load additive matrices for three markers
...
somePDFPath = "rkhsADMARO_threeMarkers.pdf"
...
for (i in 1:11) {
  ...
  fm1 = BGLR(y=yNA,ETA=list(ETA01=list(X=V1,model="FIXED"),ETA1=list(K=G_VanRadenPine1,model='RKHS'), ETA2=list(K=G_VanRadenPine2,model='RKHS'), ETA3=list(K=G_VanRadenPine3,model='RKHS')), nIter=nIter,saveAt=sprintf("./fmgRKHSaroadmThreeMarkers_y%.0f_",i),verbose=F)
  ...
  par(new=TRUE)
  plot(yHat[tstARO]~y1[tstARO], col="BLUE", xlim=tmp,ylim=c(-2,2),xlab="Observed",ylab="Predicted",main=c(names[i],sprintf("ARO,ADM ThreeMarkers RKHS, cor%.02f", corel1)))
  par(new=TRUE)
...
png(file=sprintf("./corr_RKHS_threeMarkers_AROADM.png"))
...
save.image("RKHSthreeMarkersAROADM.RData")
rm(list = ls())
...
for (i in 1:11) {
  ...
  fm2 = BGLR(y=yNA,ETA=list(ETA01=list(X=V1,model="FIXED"),ETA1=list(K=G_VanRadenPine1,model='RKHS'), ETA2=list(K=G_VanRadenPine2,model='RKHS'), ETA3=list(K=G_VanRadenPine3,model='RKHS')), nIter=nIter,saveAt=sprintf("./fmgRKHSindALL_y%.0f_",i),verbose=F)
  ...
  y1=as.numeric(y)
  tmp<-range(c(yHat,y),na.rm = T) 
  plot(yHat[tst]~ y1[tst], col="red", xlim=tmp,ylim=c(-2,2),xlab="Observed",ylab="Predicted",main=c(names[i],sprintf("IND ALL RKHS, cor%.02f", corel2)))
  par(new=TRUE)
...
row.names(all.phenotypes.COR)<-names
colnames(all.phenotypes.COR)<- c("correlation")
png(file=sprintf("./corr_RKHS_threeMarkers_IND.png"))
grid.table(all.phenotypes.COR)

dev.off()


save.image("RKHSthreeMarkersIND.RData")
rm(list = ls())
