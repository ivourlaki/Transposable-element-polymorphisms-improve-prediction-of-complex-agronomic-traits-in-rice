...
for (i in 1:11) {
  ...
  fm1 = BGLR(y=yNA,ETA=list(ETA01=list(X=V1,model="FIXED"),ETA1=list(X=mite, model='BayesC', probIn=pi, counts=p0)), nIter=nIter,saveAt=sprintf("./fmgBAYESCaroadmmite_y%.0f_",i),verbose=F)
  ...
  par(new=TRUE)
  plot(yHat[tstARO]~y1[tstARO], col="BLUE", xlim=tmp,ylim=c(-2,2),xlab="Observed",ylab="Predicted",main=c(names[i],sprintf("ARO,ADM mite BayesC, cor%.02f", corel1)))
  par(new=TRUE)
...
png(file=sprintf("./corr_BAYESC_mite_AROADM.png"))
...
save.image("BAYESCmiteAROADM.RData")
rm(list = ls())
...
somePDFPath = "bayesCIND_mite.pdf"
pdf(file=somePDFPath, onefile=TRUE)  
par(mfrow=c(2,2))
...
for (i in 1:11) {
  ...
  fm2 = BGLR(y=yNA,ETA=list(ETA01=list(X=V1,model="FIXED"),ETA1=list(X=mite, model='BayesC', probIn=pi, counts=p0)), nIter=nIter,saveAt=sprintf("./fmgBAYESCINDmite_y%.0f_",i),verbose=F)
  ...
  y1=as.numeric(y)
  tmp<-range(c(yHat,y),na.rm = T) 
  plot(yHat[tst]~ y1[tst], col="red", xlim=tmp,ylim=c(-2,2),xlab="Observed",ylab="Predicted",main=c(names[i],sprintf("IND mite BayesC, cor%.02f", corel2)))
  par(new=TRUE)
...
row.names(all.phenotypes.COR)<-names
colnames(all.phenotypes.COR)<- c("correlation")
png(file=sprintf("./corr_BAYESc_mite_IND.png"))
grid.table(all.phenotypes.COR)
