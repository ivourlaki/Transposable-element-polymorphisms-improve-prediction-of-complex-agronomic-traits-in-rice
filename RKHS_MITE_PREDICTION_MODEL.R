...
###  load additive matrix but use only the second one for MITE
...
somePDFPath = "rkhsADMARO_MITE.pdf"
...
for (i in 1:11) {
  ...
  fm1 = BGLR(y=yNA,ETA=list(ETA01=list(X=V1,model="FIXED"),ETA1=list(K=G_VanRadenPine2,model='RKHS')), nIter=nIter,saveAt=sprintf("./fmgRKHSaroadmMITE_y%.0f_",i),verbose=F)
  ...
  par(new=TRUE)
  plot(yHat[tstARO]~y1[tstARO], col="BLUE", xlim=tmp,ylim=c(-2,2),xlab="Observed",ylab="Predicted",main=c(names[i],sprintf("ARO,ADM MITE RKHS, cor%.02f", corel1)))
  par(new=TRUE)
...
png(file=sprintf("./corr_RKHS_MITE_AROADM.png"))
...
save.image("RKHSMITEAROADM.RData")
rm(list = ls())
...
for (i in 1:11) {
  ...
  fm2 = BGLR(y=yNA,ETA=list(ETA01=list(X=V1,model="FIXED"),ETA1=list(K=G_VanRadenPine2,model='RKHS')), nIter=nIter,saveAt=sprintf("./fmgRKHSindMITE_y%.0f_",i),verbose=F)
  ...
  y1=as.numeric(y)
  tmp<-range(c(yHat,y),na.rm = T) 
  plot(yHat[tst]~ y1[tst], col="red", xlim=tmp,ylim=c(-2,2),xlab="Observed",ylab="Predicted",main=c(names[i],sprintf("IND MITE RKHS, cor%.02f", corel2)))
  par(new=TRUE)
...
row.names(all.phenotypes.COR)<-names
colnames(all.phenotypes.COR)<- c("correlation")
png(file=sprintf("./corr_RKHS_MITE_IND.png"))
grid.table(all.phenotypes.COR)

dev.off()


save.image("RKHSMITEIND.RData")
rm(list = ls())
